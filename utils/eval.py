...
import numpy as np
import pandas as pd
from model_utils import sample_M
from data_utils import convert_tr_dt_to_dt
from loss import discriminator_loss, generator_loss
from metrics import calc_metrics


def eval(loader, generator, discriminator, device):
...
def eval_with_mcd(loader, num_samples=10):
    generator.eval()  # Set the generator to evaluation mode
    discriminator.eval()  # Set the discriminator to evaluation mode
    ...
    with torch.no_grad():  # No gradients needed for evaluation
        for batch_data in loader:
            ...
            print(torch.mean(variance_imputed_Y_mb))
    ...
    return imputed_data_samples, gain_data_samples, y_actual_samples, date_list, y_mask_list, all_metrics_df, variance_data_samples
